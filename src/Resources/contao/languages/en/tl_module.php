<?php

declare(strict_types=1);

/*
 * This file is part of the ContaoNewsCategoriesSearchBundle.
 *
 * (c) inspiredminds
 *
 * @license LGPL-3.0-or-later
 */

$GLOBALS['TL_LANG']['tl_module']['newscategories_legend'] = 'News categories';
$GLOBALS['TL_LANG']['tl_module']['search_enableNewsCategoriesFilter'] = ['News categories filter', 'Enables filtering search results by news categories.'];
$GLOBALS['TL_LANG']['tl_module']['search_enableTimeFilter'] = ['Timeframe filter', 'Enables filtering search results of news entries by timeframe.'];
$GLOBALS['TL_LANG']['tl_module']['search_news_sorting'] = ['News sorting', 'Sorting based on news results.'];
$GLOBALS['TL_LANG']['tl_module']['search_news_sorting_options'] = ['date_desc' => 'News date (descending)', 'date_asc' => 'News date (ascending)'];
$GLOBALS['TL_LANG']['tl_module']['search_news_sorting_blank'] = 'Relevance';
