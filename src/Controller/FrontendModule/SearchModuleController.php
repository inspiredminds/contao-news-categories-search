...
#[AsFrontendModule('search', 'application', priority: 1)]
class SearchModuleController extends ModuleSearch
{
    ...
    protected function compile(): void
    {
        ...
        if (!isset($_GET['keywords']) && 'tl_search' === Input::post('FORM_SUBMIT')) {
        ...
        $strKeywords = trim((string) Input::get('keywords'));
        ...
        $this->Template->advanced = ('advanced' === $this->searchType);
        ...
        if ('' !== $strKeywords && '*' !== $strKeywords && !$this->jumpTo) {
            ...
            try {
                $objResult = Search::query($strKeywords, 'or' === $strQueryType, $arrPages, $blnFuzzy, $this->minKeywordLength);
            } catch (\Exception $e) {
            ...
            if (Config::get('indexProtected')) {
                $objResult->applyFilter(static fn ($v) => empty($v['protected']) || System::getContainer()->get('security.helper')->isGranted(ContaoCorePermissions::MEMBER_IN_GROUPS, StringUtil::deserialize($v['groups'] ?? null, true)));
            }
            ...
            $lengths = StringUtil::deserialize($this->contextLength, true) + [null, null];
            ...
            foreach (array_keys($arrResult) as $i) {
                ...
                $objTemplate->url = StringUtil::specialchars(urldecode((string) $arrResult[$i]['url']), true, true);
                $objTemplate->title = StringUtil::specialchars(StringUtil::stripInsertTags($arrResult[$i]['title']));
                $objTemplate->class = (0 === $i ? 'first ' : '').(empty($arrResult[$i + 1]) ? 'last ' : '').(0 === $i % 2 ? 'even' : 'odd');
